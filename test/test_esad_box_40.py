#!/usr/bin/env python3
"""
Test script for eSAD Box 40 field (Previous document)
"""
...
def test_box_40_field():
    """Test Box 40 field: Previous document"""
    
    print("🧪 Testing eSAD Box 40 Field")
    print("=" * 50)
    ...
    try:
        ...
        # Test Box 40 field by processing the complete order
        order_number = "ORD-20250916-004"  # Extract from the test data path
        
        print(f"\n📋 Testing Box 40 Field:")
        print(f"   • Transport document/Previous Document")
        ...
        # Extract Box 40 result from the processing results
        if esad_result.get("status") == "success":
            esad_data = esad_result.get("esad_data", {})
            box_40_value = esad_data.get("40", {}).get("Transport document/Previous Document")
            
            if box_40_value:
                print(f"   ✅ Box 40 Result: {box_40_value}")
            else:
                print(f"   ⚠️ No data found for Box 40")
                
            print(f"\n📊 Box 40 Test Summary:")
            print(f"   • Field tested: Transport document/Previous Document")
...
if __name__ == "__main__":
    success = test_box_40_field()
    if success:
        print(f"\n🎉 Box 40 test completed successfully!")
    else:
        print(f"\n💥 Box 40 test failed!")
        sys.exit(1)
