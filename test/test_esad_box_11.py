#!/usr/bin/env python3
"""
Test script for eSAD Box 11 field (Trading Country)
"""
...
def test_box_11_field():
    """Test Box 11 field: Trading Country"""
    
    print("🧪 Testing eSAD Box 11 Field")
    print("=" * 50)
    ...
    try:
        ...
        # Test Box 11 field by processing the complete order
        order_number = "ORD-20250916-004"  # Extract from the test data path
        
        print(f"\n📋 Testing Box 11 Field:")
        print(f"   • Trading Country")
        ...
        # Extract Box 11 result from the processing results
        if esad_result.get("status") == "success":
            esad_data = esad_result.get("esad_data", {})
            box_11_value = esad_data.get("11", {}).get("Trading Country")
            
            if box_11_value:
                print(f"   ✅ Box 11 Result: {box_11_value}")
            else:
                print(f"   ⚠️ No data found for Box 11")
                
            print(f"\n📊 Box 11 Test Summary:")
            print(f"   • Field tested: Trading Country")
...
if __name__ == "__main__":
    success = test_box_11_field()
    if success:
        print(f"\n🎉 Box 11 test completed successfully!")
    else:
        print(f"\n💥 Box 11 test failed!")
        sys.exit(1)
