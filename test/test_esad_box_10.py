#!/usr/bin/env python3
"""
Test script for eSAD Box 10 field (Country Last Consignment)
"""
...
def test_box_10_field():
    """Test Box 10 field: Country Last Consignment"""
    
    print("🧪 Testing eSAD Box 10 Field")
    print("=" * 50)
    ...
    try:
        ...
        # Test Box 10 field by processing the complete order
        order_number = "ORD-20250916-004"  # Extract from the test data path
        
        print(f"\n📋 Testing Box 10 Field:")
        print(f"   • Country Last Consignment")
        ...
        # Extract Box 10 result from the processing results
        if esad_result.get("status") == "success":
            esad_data = esad_result.get("esad_data", {})
            box_10_value = esad_data.get("10", {}).get("Country Last Consignment")
            
            if box_10_value:
                print(f"   ✅ Box 10 Result: {box_10_value}")
            else:
                print(f"   ⚠️ No data found for Box 10")
                
            print(f"\n📊 Box 10 Test Summary:")
            print(f"   • Field tested: Country Last Consignment")
...
if __name__ == "__main__":
    success = test_box_10_field()
    if success:
        print(f"\n🎉 Box 10 test completed successfully!")
    else:
        print(f"\n💥 Box 10 test failed!")
        sys.exit(1)
