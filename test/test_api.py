#!/usr/bin/env python3
"""
Simple test script for the 10-stage commodity code workflow API.
Tests the complete workflow through API calls.
"""
...
def call_classify_api(product_name, contextual_data=None):
    ...
    print(f"🔍 DEBUG: Payload being sent to API:")
    print(f"   URL: {url}")
...
def test_case_3_full_context():
    """Test Case 3: Tesla with full context (should classify directly)."""
    print_separator("TEST CASE 3: 2024 Tesla Model 3 with Full Context")
    
    # Test data with full context
    product_name = "2024 Tesla Model 3"
    
    # Complete contextual data that answers all questions
    contextual_data = {
        'importer_type': 'individual',
        'product_age_category': 'three_years_and_less',
    ...
    if result:
        print_test_result(
            "Tesla with Full Context",
            result,
            "Should classify directly since all questions can be answered from context"
        )
    else:
        print(f"   ❌ API call failed")
...
def test_case_4_direct_hs_code():
    """Test Case 4: Direct HS code lookup (bypass classification)."""
    print_separator("TEST CASE 4: Direct HS Code 8703.80 Lookup")
    
    # Test data with direct HS code
    product_name = "Electric Vehicle"
    contextual_data = {
        'hs_codes': ['8703.80'],  # Direct HS code
        'importer_type': 'individual',
        'product_age_category': 'three_years_and_less'
    }
    ...
    # Test via API with direct HS code
    print(f"\n🔍 Testing via API with Direct HS Code")
    result = call_classify_api(product_name, contextual_data)
    ...
    if result:
        print_test_result(
            ...
            result,
            "Should use the 10-stage workflow with the provided HS code"
        )
    else:
        print(f"   ❌ API call failed")
...
def main():
    """Run all test cases."""
    print(f"🚀 10-STAGE COMMODITY CODE WORKFLOW API TEST SUITE")
    print(f"   Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"   Testing the complete 10-stage workflow through API")
    ...
    print(f"✅ Test Cases Completed: {len([r for r in results.values() if r is not None])}/1")
    print(f"❌ Test Cases Failed: {len([r for r in results.values() if r is None])}/1")
    
    print(f"\n🎯 Key Validation Points:")
    print(f"   - 10-stage workflow should execute for all requests")
    print(f"   - Context resolution should work properly")
    print(f"   - Clarification requests should identify missing information")
    print(f"   - Direct classification should work with complete context")
