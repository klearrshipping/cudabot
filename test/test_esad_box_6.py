#!/usr/bin/env python3
"""
Test script for eSAD Box 6 field (Total packages)
"""
...
def test_box_6_field():
    """Test Box 6 field: Total packages"""
    
    print("🧪 Testing eSAD Box 6 Field")
    print("=" * 50)
    ...
    try:
        ...
        # Test Box 6 field by processing the complete order
        ...
        # Extract Box 6 result from the processing results
        if esad_result.get("status") == "success":
            esad_data = esad_result.get("esad_data", {})
            box_6_value = esad_data.get("6", {}).get("Total packages")
            
            if box_6_value:
                print(f"   ✅ Box 6 Result: {box_6_value}")
            else:
                print(f"   ⚠️ No data found for Box 6")
                
            print(f"\n📊 Box 6 Test Summary:")
            print(f"   • Field tested: Total packages")
...
if __name__ == "__main__":
    success = test_box_6_field()
    if success:
        print(f"\n🎉 Box 6 test completed successfully!")
    else:
        print(f"\n💥 Box 6 test failed!")
        sys.exit(1)
