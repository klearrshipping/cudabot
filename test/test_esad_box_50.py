#!/usr/bin/env python3
"""
Test script for eSAD Box 50 field (Principal and Guarantor)
"""
...
def test_box_50_field():
    """Test Box 50 field: Principal and Guarantor"""
    
    print("🧪 Testing eSAD Box 50 Field")
    print("=" * 50)
    ...
    try:
        ...
        # Test Box 50 field by processing the complete order
        order_number = "ORD-20250916-004"  # Extract from the test data path
        
        print(f"\n📋 Testing Box 50 Field:")
        print(f"   • Principal")
        ...
        # Extract Box 50 result from the processing results
        if esad_result.get("status") == "success":
            esad_data = esad_result.get("esad_data", {})
            box_50_value = esad_data.get("50", {}).get("Principal")
            
            if box_50_value:
                print(f"   ✅ Box 50 Result: {box_50_value}")
            else:
                print(f"   ⚠️ No data found for Box 50")
                
            print(f"\n📊 Box 50 Test Summary:")
            print(f"   • Field tested: Principal")
...
if __name__ == "__main__":
    success = test_box_50_field()
    if success:
        print(f"\n🎉 Box 50 test completed successfully!")
    else:
        print(f"\n💥 Box 50 test failed!")
        sys.exit(1)
