#!/usr/bin/env python3
"""
Test script for eSAD Box 8 fields (Importer/Consignee TRN No., Name/Company, and Address)
"""
...
def test_box_8_fields():
    """Test Box 8 fields: Importer/Consignee TRN No., Name/Company, and Address"""
    
    print("🧪 Testing eSAD Box 8 Fields")
    print("=" * 50)
    ...
    try:
        ...
        # Test Box 8 fields by processing the complete order
        order_number = "ORD-20250916-004"  # Extract from the test data path
        
        print(f"\n📋 Testing Box 8 Fields:")
        print(f"   • Importer/Consignee TRN No.")
        print(f"   • Importer/Consignee Name/Company")
        print(f"   • Importer/Consignee Address")
        ...
        # Extract Box 8 results from the processing results
        if esad_result.get("status") == "success":
            esad_data = esad_result.get("esad_data", {})
            box_8_data = esad_data.get("8", {})
            
            trn_no = box_8_data.get("Importer/Consignee TRN No.")
            name_company = box_8_data.get("Importer/Consignee Name/Company")
            address = box_8_data.get("Importer/Consignee Address")
            
            print(f"\n🔍 Box 8 Results:")
            if trn_no:
                print(f"   ✅ Importer/Consignee TRN No.: {trn_no}")
            else:
                print(f"   ⚠️ No data found for Importer/Consignee TRN No.")
                
            if name_company:
                print(f"   ✅ Importer/Consignee Name/Company: {name_company}")
            else:
                print(f"   ⚠️ No data found for Importer/Consignee Name/Company")
                
            if address:
                print(f"   ✅ Importer/Consignee Address: {address}")
            else:
                print(f"   ⚠️ No data found for Importer/Consignee Address")
                
            print(f"\n📊 Box 8 Test Summary:")
            print(f"   • Fields tested: 3 (TRN No., Name/Company, Address)")
...
if __name__ == "__main__":
    success = test_box_8_fields()
    if success:
        print(f"\n🎉 Box 8 test completed successfully!")
    else:
        print(f"\n💥 Box 8 test failed!")
        sys.exit(1)
