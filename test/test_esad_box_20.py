#!/usr/bin/env python3
"""
Test script for eSAD Box 20 fields (Delivery terms and Mode of transport at border)
"""
...
def test_box_20_fields():
    """Test Box 20 fields: Delivery terms and Mode of transport at border"""
    
    print("🧪 Testing eSAD Box 20 Fields")
    print("=" * 50)
    ...
    try:
        ...
        # Test Box 20 fields by processing the complete order
        order_number = "ORD-20250916-004"  # Extract from the test data path
        
        print(f"\n📋 Testing Box 20 Fields:")
        print(f"   • Delivery terms")
        print(f"   • Applicable place")
        ...
        # Extract Box 20 results from the processing results
        if esad_result.get("status") == "success":
            esad_data = esad_result.get("esad_data", {})
            box_20_data = esad_data.get("20", {})
            
            delivery_terms = box_20_data.get("Delivery terms")
            applicable_place = box_20_data.get("Applicable place")
            
            print(f"\n🔍 Box 20 Results:")
            if delivery_terms:
                print(f"   ✅ Delivery terms: {delivery_terms}")
            else:
                print(f"   ⚠️ No data found for Delivery terms")
                
            if applicable_place:
                print(f"   ✅ Applicable place: {applicable_place}")
            else:
                print(f"   ⚠️ No data found for Applicable place")
                
            print(f"\n📊 Box 20 Test Summary:")
            print(f"   • Fields tested: 2 (Delivery terms, Applicable place)")
...
if __name__ == "__main__":
    success = test_box_20_fields()
    if success:
        print(f"\n🎉 Box 20 test completed successfully!")
    else:
        print(f"\n💥 Box 20 test failed!")
        sys.exit(1)
