...
class RegimeTypeProcessor:
    ...
    def _determine_import_export(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Determine if this is import or export using high-signal fields first, then fallback text analysis."""
        
        form_fields = data.get('form_fields', {})
        bol_data = data.get('bol_data', {})
        existing = data.get('existing_fields', {})  # LLM-extracted fields from Stage 3

        home = self.home_country.lower()

        # 1) Highest priority: BOL ports
        port_discharge = str(bol_data.get('port_of_discharge', '')).lower()
        port_loading = str(bol_data.get('port_of_loading', '')).lower()
        if home in port_discharge or 'jamaica' in port_discharge:
            direction = 'Import'
            source = 'port_of_discharge'
        elif home in port_loading or 'jamaica' in port_loading:
            direction = 'Export'
            source = 'port_of_loading'
        else:
            # 2) Next: LLM-extracted addresses (importer/exporter)
            importer_addr = str(existing.get('8_importer_consignee_address', '')).lower()
            exporter_addr = str(existing.get('2_exporter_consignor_address', '')).lower()
            importer_name = str(existing.get('8_importer_consignee_name_company', ''))
            exporter_name = str(existing.get('2_exporter_consignor_name_company', ''))

            if 'jamaica' in importer_addr or home in importer_addr:
                direction = 'Import'
                source = 'importer_address'
            elif 'jamaica' in exporter_addr or home in exporter_addr:
                direction = 'Export'
                source = 'exporter_address'
            else:
                # 3) No free-text fallback: remain neutral default
                direction = 'Export'
                source = 'no_signal'

        # Build result using best-known entities
        if direction == 'Import':
            home_entity = existing.get('8_importer_consignee_name_company') or form_fields.get('consignee_name', '')
            foreign_entity = existing.get('2_exporter_consignor_name_company') or form_fields.get('shipper', '')
        else:
            home_entity = existing.get('2_exporter_consignor_name_company') or form_fields.get('shipper', '')
            foreign_entity = existing.get('8_importer_consignee_name_company') or form_fields.get('consignee_name', '')

        print(f"📦 Direction decided: {direction} (source: {source})")
        print(f"🏠 Home country entity: {home_entity}")

        return {
            'direction': direction,
            'consignee_in_jamaica': direction == 'Import',
            'jamaica_indicators': [source],
            'analyzed_text': '',
            'home_country_entity': home_entity,
    ...
    def _analyze_commercial_nature(self, data: Dict[str, Any], direction_info: Dict) -> Dict[str, Any]:
        ...
        # MANDATORY OVERRIDES - Check for vehicles and high-value items
        mandatory_commercial = self._check_mandatory_commercial_indicators(data, product_classification)
        if mandatory_commercial:
            is_commercial = True
            print(f"⚡ MANDATORY COMMERCIAL: {mandatory_commercial['reasoning']}")
        ...
        # Analyze supplier for commercial indicators
        supplier_analysis = self._analyze_supplier(form_fields)
        supplier_is_commercial = supplier_analysis['indicator'] == 'Commercial'
        ...
        commercial_analysis = {
            ...
            'is_commercial_by_supplier': supplier_is_commercial,
            'is_mandatory_commercial': mandatory_commercial is not None,
            'final_commercial_determination': is_commercial or has_commercial_keywords or is_containerized or supplier_is_commercial or (mandatory_commercial is not None),
    ...
    def _check_mandatory_commercial_indicators(self, data: Dict[str, Any], product_classification: Dict) -> Optional[Dict]:
        """Check for mandatory commercial indicators that override all other classifications"""
        
        # Check for motor vehicles in product descriptions
        products = product_classification.get('products', [])
        for product in products:
            description = product.get('description', '').lower()
            
            # Vehicle keywords that are ALWAYS commercial
            vehicle_keywords = [
                'tesla', 'bmw', 'mercedes', 'audi', 'toyota', 'honda', 'nissan', 'ford', 'chevrolet',
                'volkswagen', 'hyundai', 'kia', 'mazda', 'subaru', 'lexus', 'porsche', 'ferrari',
                'lamborghini', 'bentley', 'jaguar', 'land rover', 'range rover',
                'car', 'truck', 'suv', 'van', 'motorcycle', 'scooter', 'atv', 'boat', 'yacht',
                'vehicle', 'automobile', 'motor', 'electric vehicle', 'ev', 'hybrid'
            ]
            
            for keyword in vehicle_keywords:
                if keyword in description:
                    return {
                        'reasoning': f'Product "{product["description"]}" contains vehicle keyword "{keyword}" - MANDATORY COMMERCIAL',
                        'category': 'vehicle',
                        'matched_keyword': keyword
                    }
        
        # Check for high-value items (>$50,000)
        total_value = 0
        for product in products:
        ...
        if total_value > 50000:
            return {
                'reasoning': f'Total value ${total_value:,.2f} exceeds high-value threshold - MANDATORY COMMERCIAL',
                'category': 'high_value',
                'value': total_value
            }
    ...
    def process_with_primary_backup(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        backup_model = "openai/gpt-5-mini"
...
1. Commercial vs Personal Analysis (PRIMARY FACTOR):
   ...
   - CRITICAL: Motor vehicles (cars, trucks, SUVs, Tesla, BMW, etc.) are ALWAYS commercial
   - CRITICAL: High-value items (>$10,000) from business suppliers are ALWAYS commercial
   - CRITICAL: Tesla Model Y, BMW, Mercedes, etc. are ALWAYS commercial regardless of consignee
...
"""
    ...
    def _analyze_product_nature(self, description: str) -> Dict[str, Any]:
        ...
        commercial_indicators = [
            ...
            # Motor vehicles are inherently commercial
            'motor vehicle', 'vehicle', 'automobile', 'car', 'truck', 'suv',
            'tesla', 'bmw', 'mercedes', 'audi', 'lexus', 'porsche', 'ferrari',
            'lamborghini', 'bentley', 'rolls royce', 'maserati', 'jaguar',
            'land rover', 'range rover', 'electric vehicle', 'ev', 'hybrid'
        ]
    ...
    def _analyze_supplier(self, form_fields: Dict) -> Dict[str, Any]:
        """Analyze supplier/shipper information for commercial vs household indicators"""
        
        supplier_name = form_fields.get('shipper', '').lower()
        supplier_address = form_fields.get('shipper_address', '').lower()
        
        # Commercial indicators for suppliers
        commercial_indicators = [
            'ltd', 'limited', 'corp', 'corporation', 'inc', 'incorporated',
            'company', 'co', 'distributors', 'distributor', 'trading',
            'imports', 'exports', 'wholesale', 'retail', 'enterprise',
            ...
            'tesla', 'bmw', 'mercedes', 'audi', 'lexus', 'porsche', 'ferrari',
            'lamborghini', 'bentley', 'rolls royce', 'maserati', 'jaguar',
            'toyota', 'honda', 'nissan', 'ford', 'chevrolet', 'cadillac',
            'volkswagen', 'volvo', 'hyundai', 'kia', 'mazda', 'subaru'
        ]
        
        # Check for commercial indicators
        commercial_found = []
        for indicator in commercial_indicators:
            if indicator in supplier_name or indicator in supplier_address:
                commercial_found.append(indicator)
        
        if commercial_found:
            return {
                'indicator': 'Commercial',
                'reasoning': f'Supplier shows commercial indicators: {commercial_found}',
                'weight': 'Primary'  # Supplier type is more important than consignee
            }
        else:
            return {
                'indicator': 'Household',
                'reasoning': 'Supplier appears to be individual/residential',
                'weight': 'Primary'
            }
