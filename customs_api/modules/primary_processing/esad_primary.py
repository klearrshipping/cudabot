...
class ESADPrimaryProcessor:
    ...
    def process_order(self, order_id: int, bol_extraction: Dict[str, Any], invoice_extraction: Dict[str, Any], order_number: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            json_file_path = self._generate_esad_fields_json(order_id, esad_fields, order_number)
...
RESPONSE FORMAT: Return only the clean value as specified above."""
    ...
    def _generate_esad_fields_json(self, order_id: int, esad_fields: Dict[str, Any], order_number: str = None) -> Path:
        """
        ...
        Args:
            ...
            order_number: Order number for directory structure
            
        Returns:
            Path: Path to the generated JSON file
        """
        try:
            # Create output directory in the order's directory structure
            if order_number:
                output_dir = Path("processed_data") / "orders" / order_number / "esad_fields"
            else:
                output_dir = Path("processed_data") / "esad_fields"
